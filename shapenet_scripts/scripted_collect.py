...
def scripted_pushing_V6(env, env_name, pool, success_pool, noise=0.1, norm=False):
    observation = env.reset()
    object_ind = 0
    margin = 0.025
    actions, observations, next_observations, rewards, terminals, infos = \
    ...
    for _ in range(env.scripted_traj_len):
        ...
        object_dist_with_margin = np.linalg.norm(object_pos[:3] - env.goal[:3]) > dist_thresh
        ...
        if object_gripper_dist > dist_thresh:
            # print('approaching')
            action = (object_pos - ee_pos) * 7.0
            xy_diff = np.linalg.norm(action[:2] / 7.0)
            if "Drawer" in env._env_name:
            ...
            action = np.concatenate(
                (action, np.asarray([theta_action, 0., 0.])))
        elif not object_dist_with_margin:
            # print('push object to loc')
            scale_factor = 0.3
            # vec = env.goal[:3] - object_pos[:3]
            vec = np.random.uniform(low=0.0, high=1.0, size=(3,))
            action = vec/np.linalg.norm(vec) * scale_factor
            # print(action)
        ...
        # action += np.random.normal(scale=noise, size=(6,))
        action[:3] += np.random.normal(scale=noise, size=(3,)) / (np.array(env._pos_high)-np.array(env._pos_low))
        action[3] += np.random.normal(scale=noise*0.1)
        action[4:] += np.random.normal(scale=noise, size=(2,))
        action = np.clip(action, -1 + EPSILON, 1 - EPSILON)
...
def main(args):
    ...
    for j in tqdm(range(args.num_trajectories)):
        ...
        if args.rand_reaching and not args.pushing:
            ...
            env = PushingWrapperEnv(env)
            success = False
            if args.rand_reaching:
                random_reaching_railrl(env, railrl_pool, railrl_success_pool, with_theta=args.theta)
            else:
                scripted_pushing_V6(env, args.env, railrl_pool, railrl_success_pool)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pushing',  action='store_true')
