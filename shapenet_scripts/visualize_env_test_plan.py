...
model = torch.load('/2tb/home/patrickhaoy/iros_2022_sim_runs/1/itr_0.pt') ## CHANGE
policy = model['trainer/policy'].to('cuda:0')
model = io_util.load_model('/2tb/home/patrickhaoy/iros_2022_sim_runs/pretrained_gcp')
vqvae = model['vqvae'].to('cuda:0')
#itr_0_plan = np.load('/2tb/home/patrickhaoy/iros_2022_sim_runs/2/itr_0_plan.npy').squeeze() ## CHANGE
itr_150_plan = np.load('/2tb/home/patrickhaoy/iros_2022_sim_runs/1_gcp/itr_150_plan.npy').squeeze() ## CHANGE
...
for i in range(num_traj):
    ...
    for t in range(ts):
        ...
        idx = min(t // 30, 7)
        subgoal = itr_150_plan[idx].reshape(1, 720) ## CHANGE
...
if save_video:
    writer = skvideo.io.FFmpegWriter(video_save_path + "itr_150_48.mp4") ## CHANGE
    for i in range(num_traj*ts):
        writer.writeFrame(observations[i, :, :, :])
    writer.close()
    np.save(video_save_path + "itr_150_actions", actions) ## CHANGE
