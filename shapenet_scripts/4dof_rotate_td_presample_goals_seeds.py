...
test_env_seeds = args.test_env_seeds[5:
                                     6] if args.debug else args.test_env_seeds
...
for test_env_seed in test_env_seeds:
    if args.debug:
        ...
        data_save_path = "/2tb/home/patrickhaoy/data/affordances/data/new_close_view_antialias_reset_free_v5_rotated_semicircle_top_drawer/{}top_drawer_goals_seed{}.pkl".format(
            "full_open_close_" if args.full_open_close_init_and_goal else "", str(test_env_seed))
    ...
    for i in tqdm(range(num_trajectories)):
        ...
        for k in range(40):
            action = env.get_demo_action(
                first_timestep=(k == 0), final_timestep=False)
            obs, reward, done, info = env.step(action)

        for j in range(args.num_timesteps):
            action = env.get_demo_action(
                first_timestep=False, final_timestep=(j == args.num_timesteps-1))
            obs, reward, done, info = env.step(action)
