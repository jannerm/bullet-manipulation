...
class SawyerRigAffordancesV7(SawyerBaseEnv):
    ...
    def _load_table(self):
        ...
        if self.drawer_sliding:
            ...
            rgbas = [[v / 255.0 for v in list(random.choices(range(256), k=3))] + [1] for _ in range(4)]
            self._top_drawer = bullet.objects.drawer_lightblue_base_longhandle_rgba(
                quat=quat, pos=drawer_frame_pos, rgba=rgbas, physicsClientId=self._uid, scale=drawer_scale)
        ...
        rgba = [v / 255.0 for v in list(random.choices(range(256), k=3))] + [1]
        self._top_drawer_tray = bullet.objects.tray_teal_rgba(
            quat=quat, pos=top_drawer_tray_pos, scale=0.165, rgba=rgba, physicsClientId=self._uid)
    ...
    def spawn_object(self, object_position=None, quat=None, rgba=[0, 1, 0, 1], scale=2):
        ...
        rgba = [v / 255.0 for v in list(random.choices(range(256), k=3))] + [1]
        scale = random.uniform(1.25, 2.25)
    ...
    def move_obj_slide(self, print_stages=False):
        ...
        ee_early_stage_goal_pos = obj_pos - 0.09 * \
