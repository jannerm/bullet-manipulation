...
class FrankaGraspV2Env(FrankaBaseEnv):
    ...
    def step(self, action):
        ...
        pos = list(bullet.get_link_state(self._franka, self._end_effector, 'pos'))
        delta_pos = action[:3]
        pos += delta_pos * self._action_scale
        pos = np.clip(pos, self._pos_low, self._pos_high)

        theta = list(bullet.get_link_state(self._franka, self._end_effector, 'theta'))
        ...
        if pos[2] < self._height_threshold:
            ...
            for i in range(10):
                self._simulate(pos, target_theta, gripper)
            for i in range(50):
                pos = bullet.get_link_state(self._franka, self._end_effector,
                                            'pos')
                pos = list(pos)
                pos = np.clip(pos, self._pos_low, self._pos_high)
                pos[2] += 0.05
...
if __name__ == "__main__":
    ...
    num_objects = 5
    env = roboverse.make("FrankaGraspV2-v0",
                         gui=True,
                         observation_mode='state',)
