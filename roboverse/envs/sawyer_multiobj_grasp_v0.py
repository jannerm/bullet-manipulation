...
# SHAPENET_ASSET_PATH = osp.join(osp.dirname(osp.abspath(__file__)), '../bullet-objects/ShapeNetCore')
...
class SawyerMultiobjGraspV0Env(SawyerBaseEnv):
    ...
    def __init__(self,
        ...
        new_object_list = []
        for object_path in self.object_list:
            path = object_path.split('/')
            dir_name = path[-2]
            object_name = path[-1]
            full_path = SHAPENET_ASSET_PATH + '/ShapeNetCore_vhacd/{0}/{1}/model.obj'.format(
                    dir_name, object_name)
            if osp.exists(full_path):
                new_object_list.append(object_path)
