...
import pybullet
from roboverse.envs import kuka
...
class KukaGraspingProceduralEnv(gym.Env):
  ...
  def __init__(
    ...
    if render_mode == 'GUI':
      ...
      self.cid = pybullet.connect(pybullet.TCP, 'localhost', 7772)
    ...
    if self._simple_obs:
      ...
      pos_space = spaces.Box(low=-5, high=5, shape=(8,))
  ...
  def _get_observation(self):
    if self._simple_obs:
      ...
      left_tip_pos = bullet.get_link_state(self._kuka.kukaUid, 'left_base_tip_joint', keys='pos')
      right_tip_pos = bullet.get_link_state(self._kuka.kukaUid, 'right_base_tip_joint', keys='pos')
      left_tip_pos = np.asarray(left_tip_pos)
      right_tip_pos = np.asarray(right_tip_pos)
      gripper_tips_distance = [np.linalg.norm(left_tip_pos - right_tip_pos)]
    
      end_effector_pos = list(bullet.get_link_state(self._kuka.kukaUid,self._kuka.kukaEndEffectorIndex, 'pos'))
      end_effector_theta = list(bullet.get_link_state(self._kuka.kukaUid, self._kuka.kukaEndEffectorIndex, 'theta', quat_to_deg=False))
      state_observation = np.concatenate((end_effector_pos, end_effector_theta, gripper_tips_distance))
      
      return dict(image=np.float32(self._get_image_observation().flatten())/255.0, state = state_observation)
