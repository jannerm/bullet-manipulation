...
class SawyerGraspV2Env(SawyerBaseEnv):
    ...
    def __init__(self,
        ...
        trimodal_positions1 = [(0.49999162183937296, -1.760392168808169e-05 - 0.5, -.36),
                               (0.49999162183937296 + 0.4, -1.760392168808169e-05 - 0.2, -.36),
                               (0.49999162183937296 + 0.2, -1.760392168808169e-05 + 0.1, -.36)]

        trimodal_positions2 = [(0.49999162183937296 + 0.2, -1.760392168808169e-05 + 0.2, -.36),
                               (0.49999162183937296 - 0.05, -1.760392168808169e-05 - 0.55, -.36),
                               (0.49999162183937296 + 0.05, -1.760392168808169e-05 - 0.1, -.36)
                               ]
        ...
        self._view_matrix_obs = bullet.get_view_matrix(
            target_pos=[.75, -.10, -0.2], distance=0.85,
            yaw=90, pitch=-70, roll=0, up_axis_index=2)
    ...
    def _load_meshes(self):
        ...
        if self.trimodal:
            ...
            self._objects = {
                0: bullet.objects.cube(pos=object_positions[0], rgba=[1, 0, 0, 1], scale=0.045),
                1: bullet.objects.cube(pos=object_positions[1], rgba=[0, 1, 0, 1], scale=0.045),
                2: bullet.objects.cube(pos=object_positions[2], rgba=[0, 1, 1, 1], scale=0.045)
            }
        else: #single
            if self.randomize:
        ...
        for idx in indexes:
            ...
            self._objects = {
                key_idx: bullet.objects.lego(pos=object_positions[idx])
            }
            for _ in range(200):
                bullet.step()
        """
