...
class pointMassEnv(gym.GoalEnv):
    ...
    def reset_object_pos(self, pos=None):
        ...
        if pos is None:
            pos = [self.crop(self.np_random.uniform(low=-self.TARG_LIMIT, high=self.TARG_LIMIT), self.TARG_MIN), # can't be smaller than targ_min
    ...
    # TODO replace with cleaner code4
    ...
    def step(self, action):
        ...
        r = self.compute_reward(state['achieved_goal'], state['desired_goal'])
    ...
    def get_info(self):
        agent_pos = np.asarray(self._p.getBasePositionAndOrientation(self.mass)[0])
        obj_pos = np.asarray(self._p.getBasePositionAndOrientation(self.object)[0])
        object_gripper_distance = np.linalg.norm(agent_pos - obj_pos)
        info = {
    ...
    def render_obs(self):
        return self._p.getCameraImage(128, 128)
    ...
    def get_observation(self):
        ...
        if self._observation_mode == 'state':
            ...
            image_observation = self.render_obs()[2]
    ...
    def reset(self):
        ...
        if self.physics_client_active == 0:
            ...
            self.mass_cid = self._p.createConstraint(self.mass, -1, -1, -1, self._p.JOINT_FIXED, [0, 0, 0], [0, 0, 0],
            ...
            if self.isRender:
                ...
                self.goal_cid = self._p.createConstraint(self.goal, -1, -1, -1, self._p.JOINT_FIXED, [1, 1, 1.4],
                                                         [0, 0, 0], relativeChildPosition, relativeChildOrientation)
        ...
        state = self.calc_state()
        self.last_target_distance = self.calc_target_distance(state['achieved_goal'], state['desired_goal'])
    ...
    def get_object_midpoint(self, obj_name):  ## currently only have one object
        return np.array(self._p.getBasePositionAndOrientation(self.object)[0])

    def get_end_effector_pos(self): ## equivalent to returning the position of myself
        return np.array(self._p.getBasePositionAndOrientation(self.mass)[0])
...
class pointMassEnvObject(pointMassEnv):
    def __init__(self,
        ...
        super().__init__(render=render, use_object=use_object, sparse=sparse, TARG_LIMIT=TARG_LIMIT, *args, **kwargs)
