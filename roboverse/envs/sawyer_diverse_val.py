...
class SawyerDiverseVAL(SawyerBaseEnv):
    ...
    def _load_new_env_config(self):
        if self.use_target_config:
            ...
            rgbs = np.hstack((np.random.uniform(0, 1, (2, 3)), np.ones((2, 1))))
    ...
    def _load_table(self):
        ...
        self._reset_lego_position = np.array([.775, s * 0.125, -.25]) + self.configs['table_pos_offset']
        self.init_lego_pos = np.array([0.59, s * 0.125, -0.31]) + self.configs['table_pos_offset']
        ...
        if self.affordance_dict['drawer']:
            self._bottom_drawer = bullet.objects.drawer_no_handle(
                    pos=np.array([0.6, s * 0.125, -.34] + self.configs['table_pos_offset']), rgba=self.sample_object_color(), physicsClientId=self._uid)
            self._objects['lego'] = bullet.objects.drawer_lego(pos=self.init_lego_pos, physicsClientId=self._uid)
            if self.affordance_dict['drawer_open']:
                open_drawer(self._bottom_drawer, physicsClientId=self._uid)
            self.init_drawer_pos = get_drawer_bottom_pos(self._bottom_drawer, physicsClientId=self._uid)[0]
        ...
        if self.affordance_dict['handle_drawer']:
            ...
            if self.affordance_dict['drawer']:
                self._top_drawer = bullet.objects.drawer(quat=quat, pos=np.array([0.6, s * 0.125, -.22] + self.configs['table_pos_offset']),
                    rgba=self.sample_object_color(), physicsClientId=self._uid)
            else:
                self._top_drawer = bullet.objects.drawer(quat=quat, pos=np.array([0.6, s * 0.125, -.34] + self.configs['table_pos_offset']),
                    rgba=self.sample_object_color(), physicsClientId=self._uid)
            ...
            open_drawer(self._top_drawer, num_ts=self.drawer_opening_num_ts, physicsClientId=self._uid)
        ...
        if self.affordance_dict['button']:
            ...
            if num_drawers == 2:
                pos = np.array([0.6, s * 0.125, -.14] + self.configs['table_pos_offset'])
            elif num_drawers == 1:
                pos = np.array([0.6, s * 0.125, -.25] + self.configs['table_pos_offset'])
            else:
                pos = np.array([0.6, s * 0.125, -.34] + self.configs['table_pos_offset'])

            self.button_pos = pos
            self.button_color = self.sample_object_color()
            self._objects['button'] = bullet.objects.button(pos=self.button_pos, rgba=self.button_color, physicsClientId=self._uid)
            self.init_button_height = get_button_cylinder_pos(self._objects['button'], physicsClientId=self._uid)[2]
            self.button_used = False

        self.tray_corners = {
            "back_left" : np.array([0.6, s * -0.15, -.35] + self.configs['table_pos_offset']),
            "back_right" : np.array([0.6, s * 0.15, -.35] + self.configs['table_pos_offset']),
            "front_left" : np.array([0.79, s * -0.12, -.35] + self.configs['table_pos_offset']),
            "front_right" : np.array([0.79, s * 0.12, -.35] + self.configs['table_pos_offset'])
        }
        self.rand_obj_corners = {
            "back_left" : np.array([0.6, s * -0.15, -.25] + self.configs['table_pos_offset']),
            "back_right" : np.array([0.6, s * 0.15, -.25] + self.configs['table_pos_offset']),
            "front_left" : np.array([.78, s * -0.12, -.25] + self.configs['table_pos_offset']),
            "front_right" : np.array([.78, s * 0.12, -.25] + self.configs['table_pos_offset'])
        }
        ...
        if self.affordance_dict['tray']:
            ...
            self.tray = bullet.objects.drawer_tray(pos=tray_pos, rgba=self.sample_object_color(), physicsClientId=self._uid)
    ...
    def button_pressed(self):
        ...
        curr_height = get_button_cylinder_pos(self._objects['button'], 
                physicsClientId=self._uid)[2]
    ...
    def check_obj_bounding_box(self, obj):
        object_pos = bullet.get_body_info(obj, 
                physicsClientId=self._uid)['pos']
    ...
    def step(self, *action):
        ...
        if self.final_timestep:
            if self.affordance_dict['button']:
                p.removeBody(self._objects['button'], physicsClientId=self._uid)
                self._objects['button'] = bullet.objects.button(pos=self.button_pos, rgba=self.button_color, physicsClientId=self._uid)
                self.button_used = False
    ...
    def fancy_render_obs(self):
        ...
        fancy_projection_matrix_obs = bullet.get_projection_matrix(
            fancy_obs_dim, fancy_obs_dim, physicsClientId=self._uid)

        img, depth, segmentation = bullet.render(
            fancy_obs_dim, fancy_obs_dim, self._view_matrix_obs,
            fancy_projection_matrix_obs, shadow=0, gaussian_width=0, physicsClientId=self._uid)
        if self._transpose_image:
            img = np.transpose(img, (2, 0, 1))
        return img
    ...
    def get_observation(self):
        ...
        if self.affordance_dict['rand_obj']:
            object_pos = bullet.get_body_info(self._objects['obj'], quat_to_deg=False, physicsClientId=self._uid)['pos']
        else:
            object_pos = [0,0,0]

        if self.affordance_dict['drawer']:
            bottom_drawer_pos = [get_drawer_bottom_pos(self._bottom_drawer)[0]]
            lego_pos = bullet.get_body_info(self._objects['lego'], quat_to_deg=False, physicsClientId=self._uid)['pos']
        else:
            bottom_drawer_pos = [0]
            lego_pos = [0,0,0]

        if self.affordance_dict['handle_drawer']:
            top_drawer_pos = [get_drawer_handle_pos(self._top_drawer, physicsClientId=self._uid)[1]]
        else:
            top_drawer_pos = [0]

        if self.affordance_dict['button']:
            button_height = [get_button_cylinder_pos(self._objects['button'], physicsClientId=self._uid)[2]]
        else:
            button_height = [0]
