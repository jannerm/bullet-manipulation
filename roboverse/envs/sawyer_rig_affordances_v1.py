...
from bullet_objects import loader
from bullet_objects import metadata
from roboverse.envs.sawyer_base import SawyerBaseEnv
from roboverse.bullet import drawer_utils
# from roboverse.bullet import button_utils
...
class SawyerRigAffordancesV1(SawyerBaseEnv):
    ...
    def __init__(self,
        ...
        self.test_env_seed = kwargs.pop(
            'test_env_seed', None) if self.test_env else None
        self.test_env_seeds = kwargs.pop(
            'test_env_seeds', None) if self.test_env else None
        ...
        if self.new_view:
            if self.close_view:
                p = .9
                gripper_bounding_x[1] = (
                    (1-p) * gripper_bounding_x[0] + p * gripper_bounding_x[1])
        ...
        self.fix_drawer_orientation = kwargs.pop(
            'fix_drawer_orientation', False)
        self.fix_drawer_orientation_semicircle = kwargs.pop(
            'fix_drawer_orientation_semicircle', False)
        assert not (
            self.fix_drawer_orientation and self.drawer_orientation_semicircle)
    ...
    def get_object_info(self):
        complete_object_dict = metadata.obj_path_map
        scaling = metadata.path_scaling_map
    ...
    def _load_table(self):  # NOQA
        ...
        if self.test_env and not self.test_env_seed:
            ...
            while(True):
                drawer_frame_pos = np.array(
                    [random.uniform(gripper_bounding_x[0],
                                    gripper_bounding_x[1]),
                     random.uniform(gripper_bounding_y[0],
                                    gripper_bounding_y[1]),
                     -.34])
                drawer_handle_open_goal_pos = (
                    drawer_frame_pos + td_open_coeff * np.array(
                        [np.sin(self.drawer_yaw * np.pi / 180), -
                         np.cos(self.drawer_yaw * np.pi / 180), 0]))

                if (gripper_bounding_x[0] <= drawer_handle_open_goal_pos[0]
                                          <= gripper_bounding_x[1] and
                        gripper_bounding_y[0] <= drawer_handle_open_goal_pos[1]
                                              <= gripper_bounding_y[1]):
                    break

        quat = bullet.deg_to_quat([0, 0, self.drawer_yaw])

        if self.drawer_sliding:
            if self.red_drawer_base:
                self._top_drawer = bullet.objects.drawer_sliding_red_base(
                    quat=quat,
                    pos=drawer_frame_pos,
                    rgba=self.sample_object_color())
            else:
                self._top_drawer = bullet.objects.drawer_sliding(
                    quat=quat,
                    pos=drawer_frame_pos,
                    rgba=self.sample_object_color())
        else:
            if self.red_drawer_base:
                self._top_drawer = bullet.objects.drawer_red_base(
                    quat=quat,
                    pos=drawer_frame_pos,
                    rgba=self.sample_object_color())
            else:
        ...
        # case: uniform random drawer initialization/goal
            ...
            drawer_utils.open_drawer(
                self._top_drawer,
                num_ts=np.random.random_integers(low=1, high=60))

        self.init_handle_pos = drawer_utils.get_drawer_handle_pos(
            self._top_drawer)[1]
    ...
    def sample_quat(self, object_name):
        ...
        return bullet.deg_to_quat(np.random.randint(0, 360, size=3))
    ...
    def _format_action(self, *action):
        if len(action) == 1:
            delta_pos, delta_yaw, gripper = (
                action[0][:3], action[0][3:4], action[0][-1])
        elif len(action) == 3:
    ...
    def step(self, *action):
        ...
        default_angle = bullet.quat_to_deg(self.default_theta)
        ...
        theta = bullet.deg_to_quat(angle)
    ...
    def get_success_metric(self,
        ...
        if key == 'top_drawer':
            ...
            deg = bullet.quat_to_deg(curr_state[3:7])
            goal_deg = bullet.quat_to_deg(goal_state[3:7])
            ...
            if key == 'gripper_position':
                ...
                success = int(np.sqrt(
                    self.norm_deg(deg[0], goal_deg[0])**2 + self.norm_deg(
                        deg[1], goal_deg[1])**2 +
                    self.norm_deg(deg[2], goal_deg[2])**2)
                    < self.gripper_rot_thresh)
            elif key == 'gripper':
                success = (
                    int(np.linalg.norm(pos - goal_pos)
                        < self.gripper_pos_thresh) and
                    int(np.sqrt(self.norm_deg(deg[0], goal_deg[0])**2 +
                                self.norm_deg(deg[1], goal_deg[1])**2 +
                                self.norm_deg(deg[2], goal_deg[2])**2)
                        < self.gripper_rot_thresh))
    ...
    def get_distance_metric(self,
        ...
        if key == 'top_drawer':
            ...
            if key == 'gripper_position':
                ...
                distance = np.sqrt(
                    self.norm_deg(deg[0], goal_deg[0])**2 +
                    self.norm_deg(deg[1], goal_deg[1])**2 +
                    self.norm_deg(deg[2], goal_deg[2])**2)
    ...
    def norm_deg(self, deg1, deg2):
        return min(np.linalg.norm((360 + deg1 - deg2) % 360),
                   np.linalg.norm((360 + deg2 - deg1) % 360))

    def get_gripper_deg(self,
                        curr_state,
                        roll_list=None,
                        pitch_list=None,
                        yaw_list=None):
        quat = curr_state[3:7]
        deg = bullet.quat_to_deg(quat)
    ...
    def get_contextual_diagnostics(self, paths, contexts):  # NOQA
        ...
        for i in range(len(paths)):
            for j in range(len(paths[i]['observations'])):
                curr_obs = paths[i]['observations'][j][state_key]
                self.get_gripper_deg(
                    curr_obs,
                    roll_list=gripper_rotation_roll_list,
                    pitch_list=gripper_rotation_pitch_list,
                    yaw_list=gripper_rotation_yaw_list)

        diagnostics.update(create_stats_ordered_dict(
            state_key + '/gripper_rotation_roll',
            gripper_rotation_roll_list))
        diagnostics.update(create_stats_ordered_dict(
            state_key + '/gripper_rotation_pitch',
            gripper_rotation_pitch_list))
        diagnostics.update(create_stats_ordered_dict(
            state_key + '/gripper_rotation_yaw',
            gripper_rotation_yaw_list))
    ...
    def get_observation(self):
        ...
        hand_theta = bullet.get_link_state(
            self._sawyer,
            self._end_effector,
            'theta',
            quat_to_deg=False)
    ...
    def get_drawer_handle_future_pos(self, coeff):
        drawer_frame_pos = drawer_utils.get_drawer_frame_pos(self._top_drawer)
        return (
            drawer_frame_pos +
            coeff * np.array([np.sin(self.drawer_yaw * np.pi / 180),
                              -np.cos(self.drawer_yaw * np.pi / 180),
                              0])
        )
    ...
    def handle_more_open_than_closed(self):
        ...
        return (
            np.linalg.norm(drawer_handle_open_pos - drawer_handle_pos) <
            np.linalg.norm(drawer_handle_close_pos - drawer_handle_pos))
    ...
    def update_drawer_goal(self):
        ...
        # case: uniform random drawer initialization/goal
            ...
            if self.handle_more_open_than_closed():
                td_goal_coeff = np.random.uniform(
                    low=(td_close_coeff+td_open_coeff)/2, high=td_open_coeff)
            else:
                td_goal_coeff = np.random.uniform(
                    low=td_close_coeff, high=(td_close_coeff+td_open_coeff)/2)

        drawer_handle_goal_pos = self.get_drawer_handle_future_pos(
            td_goal_coeff)
        drawer_handle_pos = self.get_td_handle_pos()

        # if sampled goal is already achieved by drawer initialization,
        # then set goal to be drawer full open/close
    ...
    def get_td_handle_pos(self):
        return np.array(drawer_utils.get_drawer_handle_pos(self._top_drawer))
    ...
    def move_drawer(self, print_stages=False):  # NOQA
        self.grip = -1
        ee_pos = self.get_end_effector_pos()
        ee_yaw = self.get_end_effector_theta()[2]

        drawer_handle_pos = self.get_td_handle_pos()
        # drawer_frame_pos = drawer_utils.get_drawer_frame_pos(
        #     self._top_drawer)
        ee_early_stage_goal_pos = (
            drawer_handle_pos - td_offset_coeff * np.array(
                [np.sin((self.drawer_yaw+180) * np.pi / 180),
                 - np.cos((self.drawer_yaw+180) * np.pi / 180),
                 0])
        )
        ...
        if self.final_timestep and print_stages:
            print('drawer_yaw: ', self.drawer_yaw, ', drawer_frame_pos: ',
                  drawer_utils.get_drawer_frame_pos(self._top_drawer))
